#!/usr/bin/env node
/**
 * Trigger forecast generation for all merchants
 * Handles authentication and provides detailed feedback
 */
...
// Get or create a valid admin token
async function getAdminToken() {
  try {
    // Try to create a fresh token
...
async function triggerMerchantForecast(merchantId, adminToken) {
  try {
...
async function main() {
  ...
  console.log('FORECAST GENERATION TRIGGER - ALL MERCHANTS');
  console.log('═'.repeat(80));
  ...
  try {
    ...
    for (const merchant of merchants) {
      ...
      const result = await triggerMerchantForecast(merchant.id, adminToken);
      ...
      if (result.success) {
        ...
        if (result.status === 403) {
          console.log('⚠️  Forbidden (403) - Permission denied');
        } else if (result.status === 401) {
          console.log('⚠️  Unauthorized (401) - Token invalid');
        } else if (result.status === 404) {
          console.log('⚠️  Not Found (404) - Endpoint missing');
        } else {
    ...
    if (successCount > 0) {
      ...
      console.log('   You can check progress with: node check_forecast_progress.js\n');
    } else {
      console.log('⚠️  No forecasts were triggered.');
      console.log('   Please verify:');
      console.log('   1. Server is running');
      console.log('   2. JWT secret is correct');
      console.log('   3. Admin endpoint is accessible\n');
    }
